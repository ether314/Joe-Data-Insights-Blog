...
import {
  ...
  jctLinesFor,
  type LayerId,
} from "@/data/fiscal-plumbing-update-202608-data";
...
// viz-plan: 202608 vs Q3 vintage Δ; interest+Code pressure; carried trust clocks; JCT waterfall; guarantee-book scatter
...
type ViewMode = "deltas" | "code-pressure" | "guarantee-book";
...
const VIEW_MODES: { id: ViewMode; label: string }[] = [
  ...
  { id: "code-pressure", label: "Code pressure" },
  { id: "guarantee-book", label: "Guarantee book" },
];

const PIE_COLORS = ["#7c3aed", "#f59e0b", "#0ea5e9", "#ef4444", "#64748b"];

export function FiscalPlumbingUpdate202608Dashboard() {
  ...
  return (
    <div
      className="space-y-6"
      data-viz="fiscal-plumbing-update-202608"
      data-viz-dashboard
    >
      <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-5 py-5 text-white shadow-sm">
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-300">
          Versus the Q3 vintage: late-Aug CBO/Treasury framing adds{" "}
          {fmtDeltaBn(HEADLINE.netInterestDeltaBn)} of net interest, the CRFB/JCT
          late-summer path lifts the shadow budget +{HEADLINE.jctDeltaPct}%, and FHFA
          puts guaranteed GSE MBS up ${HEADLINE.gseDeltaTn.toFixed(2)}T. OASDI/HI clocks
          stay carried at {HEADLINE.oasdiNewYear}/{HEADLINE.hiNewYear}.
        </p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {(view === "deltas" || view === "code-pressure") && (
          <ChartCard
        ...
        {(view === "deltas" || view === "code-pressure") && (
          <ChartCard
            title="JCT top lines — Q3 vs late-Aug"
            subtitle="Prior Q3 newest → CRFB/JCT late-summer path. Filter by tax-expenditure family."
          >
            <div className="h-[340px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart
                ...
                >
                  ...
                  />
                  <Legend />
                  <Bar dataKey="priorBn" name="Q3 vintage" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="newBn" name="Late-Aug path" radius={[4, 4, 0, 0]}>
                    {jctRows.map((r) => (
                      <Cell key={r.id} fill={r.fill} />
                    ))}
                  </Bar>
        ...
        {(view === "deltas" || view === "code-pressure") && (
          <ChartCard
            ...
            subtitle={`Editorial FY path: ratio ${HEADLINE.interestVsDiscRatioPrior} → ${HEADLINE.interestVsDiscRatioNew}. Interest keeps closing.`}
          >
        ...
        {(view === "code-pressure" || view === "deltas") && (
          <ChartCard
            title="Where the dollar deltas landed"
            subtitle="Editorial share of late-Aug plumbing-layer dollar moves (interest ≈ Code)."
          >
            <div className="h-[320px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={2}
                    label={(props) => {
                      const p = props as { label?: string; sharePct?: number; name?: string };
                      return `${p.label ?? p.name ?? ""} ${p.sharePct ?? ""}%`;
                    }}
                  >
                    {DELTA_SHARE.map((r, i) => (
                      <Cell key={r.id} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip
                    formatter={(v, _n, item) => {
                      const row = item?.payload as (typeof DELTA_SHARE)[0] | undefined;
                      return [
                        `${fmtDeltaBn(Number(v ?? 0))} (${row?.sharePct ?? "?"}%)`,
                        row?.label ?? "Δ",
                      ];
                    }}
                  />
        ...
        {(view === "code-pressure" || view === "deltas") && (
          <ChartCard
            title="JCT aggregate waterfall (+$60B)"
            subtitle="How the late-Aug path rebuilds $2.51T → $2.57T. Floating bars = signed contributions."
          >
            <div className="h-[320px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                ...
                >
                  ...
                  <Tooltip
                    formatter={(_v, _n, item) => {
                      ...
                      return [
                        fmtDeltaBn(JCT_WATERFALL.find((j) => j.id === row.id)?.valueBn ?? 0),
                        "Δ",
                      ];
                    }}
                  />
        ...
        {(view === "guarantee-book" || view === "deltas") && (
          <ChartCard
            title="Trust-fund clocks — carried years, softer reserves"
            subtitle="Depletion years unchanged vs Q3. Solid = late-Aug reserve path; dashed = Q3."
          >
            <div className="h-[320px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <ChartCard
          title="Plumbing layers — Q3 vs late-Aug ($B)"
          ...
          <div className="h-[320px] w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={layerBars} margin={{ top: 8, right: 12, left: 0, bottom: 40 }}>
                ...
                <Legend />
                <Bar dataKey="priorBn" name="Q3 vintage" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                <Bar dataKey="newBn" name="Late-Aug newest" fill="#7c3aed" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </ChartCard>
      ...
      <ChartCard
        ...
        subtitle="Same employer-health concept, two offices. Gap narrows slightly again (49 → 47)."
      >
