/**
 * Capex intensity concentration — top-1 / top-3 shares of the intensity
 * distribution, excess-reinvestment ladder, and FCF sustainability across
 * Big-5 hyperscalers. Complements absolute-dollar spend concentration with
 * a *ratio* lens: who sits at the extreme of reinvestment / revenue.
 */

export type Confidence = "disclosed" | "estimated" | "annualized" | "guide" | "desk";

export const SOURCE_NOTE =
  "Concentration lens on Big-5 hyperscaler *capex intensity* (gross PP&E purchases ÷ total revenue). Late-Aug 202608 vintage aligned with the companion intensity update: Oracle 43.5%, Meta 40.5%, Alphabet 28.3%, Microsoft 26.8%, Amazon 23.8%. Shares and HHI are within the intensity-sum perimeter (each firm’s intensity as a share of the five intensities), not of global IT capex. Excess intensity = intensity − 11% pre-AI cloud norm. FCF margin = (OCF − capex) ÷ revenue.";

export const SOURCES = [
  {
    label: "Late-Aug intensity update (202608)",
    url: "/blog/ai-capex-intensity-update-202608",
  },
  {
    label: "Q3 intensity mid-guide",
  ...
  {
    label: "Spend concentration companion",
...
/** Late-Aug headline (intensity-sum perimeter) */
const _aug = concentrationMetrics("aug-20", "intensity-sum");
const _augExcess = concentrationMetrics("aug-20", "excess");
const _augDollars = concentrationMetrics("aug-20", "capex-dollars");

export const HEADLINE = {
  top1SharePct: _aug.top1,
  top1Label: _aug.top1Label,
  top1IntensityPct: _aug.top1Intensity,
  top3SharePct: _aug.top3,
  top3Labels: _aug.top3Labels,
  hhi: _aug.hhi,
  equalShareHhi: 2000,
  intensitySum: _aug.intensitySum,
  revenueWeightedPct: _aug.revenueWeightedPct,
  excessTop1SharePct: _augExcess.top1,
  excessTop3SharePct: _augExcess.top3,
  dollarTop1SharePct: _augDollars.top1,
  dollarTop1Label: _augDollars.top1Label,
  dollarTop3SharePct: _augDollars.top3,
  metaIntensity: 40.5,
  oracleIntensity: 43.5,
  amazonIntensity: 23.8,
  amazonFcf: 0.3,
  microsoftFcf: 20.9,
...
export const SHARE_LADDER_AUG20 = shareLadder("aug-20", "intensity-sum");
...
export function sustainabilityScatter(
  vintage: IntensityVintageKey = "aug-20",
): SustainabilityPoint[] {
...
/** Excess-intensity slices for donut (late-Aug) */
export const EXCESS_SLICES = shareLadder("aug-20", "excess").map((r) => ({
  company: r.company,
  value: r.excessPct,
  sharePct: r.sharePct,
  fill: r.fill,
...
/** Dual-perimeter comparison: intensity-sum vs dollar shares (late-Aug) */
export const PERIMETER_COMPARE = COMPANIES.map((company) => {
  const intensity = SHARE_LADDER_AUG20.find((r) => r.company === company)!;
  const dollars = shareLadder("aug-20", "capex-dollars").find(
    (r) => r.company === company,
  )!;
  return {
    company,
