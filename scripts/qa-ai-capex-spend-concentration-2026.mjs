/**
 * QA: ai-capex-spend-concentration-2026
...
} from "../src/data/ai-capex-spend-concentration-2026-data.ts";
...
assert.equal(SHARE_LADDER_2026[0].company, "Amazon");
assert.equal(SHARE_LADDER_2026[0].sharePct, 28.0);
assert.ok(POSITIVE_RAISE_SLICES.length >= 3);

const root = process.cwd();
const slug = "ai-capex-spend-concentration-2026";
const markers = [
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('[data-viz="ai-capex-spend-concentration-2026"]', {
      timeout: 20000,
    });
    ...
    console.log("qa-ai-capex-spend-concentration-2026: PASS");
  } finally {
    await browser.close();
