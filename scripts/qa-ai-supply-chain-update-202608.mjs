/**
 * QA: ai-supply-chain-update-202608
...
const slug = "ai-supply-chain-update-202608";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-supply-chain-update-202608-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiSupplyChainUpdate202608Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "industry-ai-supply-chain-update-202608-hero.png",
);

const markers = [
  "Vintage delta — Q3 CoWoS tracker → Aug Mid-Year back-end cut",
  "Back-end tool $ vs CoWoS gap",
  "Upstream dollars → downstream gate",
  "Equipment dollars — 2025 actual → 2026 Mid-Year",
  "Tightness score Δ (Q3 → Aug Mid-Year cut)",
  "Pace ladder — Q1 stamp → FY Mid-Year",
];
...
const staticChecks = [
  ...
  ["SEGMENT_PATH exported", dataSrc.includes("export const SEGMENT_PATH")],
  ["BOTTLENECK_DELTA exported", dataSrc.includes("export const BOTTLENECK_DELTA")],
  ["Headline test +31%", dataSrc.includes("testYoyPct: 31")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-supply-chain-update-202608"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Q3 CoWoS tracker") ||
      findInChunks("Back-end tool $ vs CoWoS gap"),
  ],
];
...
try {
  ...
  await page.getByRole("button", { name: "Pace / regions" }).click();
  ...
  const heroVisible = await page
    .locator('img[src*="industry-ai-supply-chain-update-202608-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
