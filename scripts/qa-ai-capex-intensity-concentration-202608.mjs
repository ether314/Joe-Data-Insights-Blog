/**
 * QA: ai-capex-intensity-concentration-202608
...
import {
  ...
  SHARE_LADDER_AUG,
  EXCESS_SLICES,
} from "../src/data/ai-capex-intensity-concentration-202608-data.ts";
...
assert.equal(SHARE_LADDER_AUG[0].company, "Oracle");
assert.equal(SHARE_LADDER_AUG[0].sharePct, 26.7);
assert.ok(EXCESS_SLICES.length >= 3);

const root = process.cwd();
const slug = "ai-capex-intensity-concentration-202608";
const dashboardPath = path.join(
  root,
  "src/components/visualizations/AiCapexIntensityConcentration202608Dashboard.tsx",
);
const dashboardSrc = fs.readFileSync(dashboardPath, "utf8");
assert.ok(
  dashboardSrc.includes('data-viz="ai-capex-intensity-concentration-202608"'),
...
async function main() {
  ...
  try {
    ...
    console.log("qa-ai-capex-intensity-concentration-202608: PASS");
  } finally {
    await browser.close();
