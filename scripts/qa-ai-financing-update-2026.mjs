...
const slug = "ai-financing-update-2026";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-financing-update-2026-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiFinancingUpdateDashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-financing-update-2026-hero.png",
);

const markers = [
  "Vintage delta — Jul research",
  "AI-related debt stack (mid-2026)",
  "Vintage deltas that moved",
  "AI share of US IG supply",
...
const staticChecks = [
  ...
  ["Headline $489B", dataSrc.includes("aiRelatedDebtYtdBn: 489")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-financing-update-2026"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Jul research") || findInChunks("AI-related debt stack"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-financing-update-2026-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
