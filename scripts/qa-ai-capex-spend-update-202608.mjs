/**
 * QA: ai-capex-spend-update-202608
 * // viz-types: waterfall, slope, stacked area YoY, revision multi-line, delta rank bars, scatter, scenario fan | layout: default
 */
...
const slug = "ai-capex-spend-update-202608";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-spend-update-202608-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexSpendUpdate202608Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-capex-spend-update-202608-hero.png",
);

const markers = [
  "202608 vintage delta — mid-Q3",
  "Q3 → Aug-20 vintage waterfall ($B)",
  "Δ contribution rank (Q3 → Aug-20)",
  "Prior → new slope (Q3 → Aug-20)",
  "Hyperscaler stack — four 2026 vintages",
  "Revision path — Jul → Aug → Q3 → Aug-20",
  "Research-house scenario fan",
];
...
const staticChecks = [
  ...
  ["COMPANY_VINTAGE exported", dataSrc.includes("export const COMPANY_VINTAGE")],
  ["HEADLINE newBig5 858", dataSrc.includes("newBig5: 858")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-spend-update-202608"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("202608 vintage delta — mid-Q3") || findInChunks("Q3 → Aug-20 vintage waterfall"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-capex-spend-update-202608-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
