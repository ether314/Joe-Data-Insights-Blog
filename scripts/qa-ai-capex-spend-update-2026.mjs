...
const slug = "ai-capex-spend-update-2026";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-spend-update-2026-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexSpendUpdateDashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-capex-spend-update-2026-hero.png",
);

const markers = [
  "Vintage delta — Jul research",
  "Company vintage change ($B)",
  "Prior → new guidance (dumbbell)",
  "Hyperscaler stack — dual 2026 vintages",
  "Research-house scenario fan",
  "Q2’26 cash absorption — capex vs OCF",
];
...
const staticChecks = [
  ...
  ["HEADLINE newBig5 802.5", dataSrc.includes("newBig5: 802.5")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-spend-update-2026"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Jul research") || findInChunks("Company vintage change"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-capex-spend-update-2026-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
