/**
 * QA: ai-capex-intensity-update-202608
...
const slug = "ai-capex-intensity-update-202608";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-intensity-update-202608-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexIntensityUpdate202608Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-capex-intensity-update-202608-hero.png",
);

const markers = [
  "Vintage delta — Q3’26 mid-guide → late-Aug 202608",
  "Q3 → Aug-20 vintage change (pp)",
  "Prior → new levels (slope bars)",
  "Intensity path — FY24 → FY25 → H1’26* → Q3’26 → Aug-20",
  "Sustainability scatter — intensity vs FCF",
  "Big-4 weighted intensity ladder",
...
const staticChecks = [
  ...
  ["Weighted delta present", dataSrc.includes("weightedDeltaPp")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-intensity-update-202608"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Q3") ||
      findInChunks("Q3 → Aug-20 vintage change"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-capex-intensity-update-202608-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
