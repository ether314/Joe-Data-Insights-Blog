/**
 * QA: ai-capex-intensity-research-2026
...
const slug = "ai-capex-intensity-research-2026";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-intensity-research-2026-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexIntensityResearchDashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "finance-ai-capex-intensity-research-2026-hero.png",
);

const markers = [
  "Capex intensity trajectory — ratio time series",
  "intensity vs free-cash-flow margin",
  "ranked capex intensity",
  "Absolute capex stack",
  "Sector capital-intensity benchmarks",
];
...
const staticChecks = [
  ...
  ["INTENSITY_SERIES exported", dataSrc.includes("export const INTENSITY_SERIES")],
  ["SECTOR_BENCHMARKS exported", dataSrc.includes("export const SECTOR_BENCHMARKS")],
  ["Meta FY25 at 34.7%", dataSrc.includes("intensityPct: 34.7")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-intensity-research-2026"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  ["Dashboard bundled", findInChunks("Capex intensity trajectory") || findInChunks("Sector capital-intensity")],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="finance-ai-capex-intensity-research-2026-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
