/**
 * QA: ai-capex-spend-update-2026q3
...
const slug = "ai-capex-spend-update-2026q3";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-spend-update-2026q3-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexSpendUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-capex-spend-update-2026q3-hero.png",
);

const markers = [
  "Q3 vintage delta — Aug post-Q2",
  "Aug → Q3 vintage waterfall ($B)",
  "Prior → new slope (Aug → Q3)",
  "Hyperscaler stack — triple 2026 vintages",
  "Revision path — Jul → Aug → Q3",
  "Q3 Big-5 composition",
...
const staticChecks = [
  ...
  ["HEADLINE newBig5 835", dataSrc.includes("newBig5: 835")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-spend-update-2026q3"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Q3 vintage delta — Aug post-Q2") || findInChunks("Aug → Q3 vintage waterfall"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-capex-spend-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
