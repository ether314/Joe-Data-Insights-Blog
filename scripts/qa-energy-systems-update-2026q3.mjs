/**
 * QA: energy-systems-update-2026q3
 * // viz-types: power-mix overtake bars, demand-attrib diverging, dual-ledger scatter, capex stack, trade exposure, capex path + companion scatter | layout: default
 */
...
const slug = "energy-systems-update-2026q3";
...
const dataPath = path.join(
  ...
  "src",
  "data",
  "energy-systems-update-2026q3-data.ts",
);
const dashboardPath = path.join(
  ...
  "components",
  "visualizations",
  "EnergySystemsUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "energy-energy-systems-update-2026q3-hero.png",
);

const markers = [
  "Renewables overtake coal in electricity",
...
const staticChecks = [
  ...
  ["POWER_MIX exported", dataSrc.includes("export const POWER_MIX")],
  ["renewPowerSharePct 33.8", dataSrc.includes("renewPowerSharePct: 33.8")],
  ["cleanInv2026Tn 2.2", dataSrc.includes("cleanInv2026Tn: 2.2")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  [
    "Dashboard wired",
    dashboardSrc.includes('data-viz="energy-systems-update-2026q3"'),
  ],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Renewables overtake coal in electricity") ||
      findInChunks("Renewables overtake coal in the power mix"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="energy-energy-systems-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
