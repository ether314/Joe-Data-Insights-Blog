/**
 * QA: ai-capex-intensity-update-2026q3
...
const slug = "ai-capex-intensity-update-2026q3";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-capex-intensity-update-2026q3-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiCapexIntensityUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-capex-intensity-update-2026q3-hero.png",
);

const markers = [
  "Vintage delta — Aug intensity update",
  "Aug → Q3 vintage change (pp)",
  "Prior → new levels (paired bars)",
...
const staticChecks = [
  ...
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-capex-intensity-update-2026q3"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Aug intensity update") ||
      findInChunks("Aug → Q3 vintage change"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-capex-intensity-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
