/**
 * QA: ai-capex-spend-concentration-202608
...
} from "../src/data/ai-capex-spend-concentration-202608-data.ts";

assert.equal(HEADLINE.top1SharePct, 28.0);
assert.equal(HEADLINE.top3SharePct, 73.8);
assert.equal(HEADLINE.big5TotalBn, 858);
assert.equal(HEADLINE.top1Bn, 240);
assert.equal(HEADLINE.hhi, 2227);
assert.equal(SHARE_LADDER_202608[0].company, "Amazon");
assert.equal(SHARE_LADDER_202608[0].sharePct, 28.0);
assert.ok(POSITIVE_RAISE_SLICES.length >= 3);

const root = process.cwd();
const slug = "ai-capex-spend-concentration-202608";
const markers = [
...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('[data-viz="ai-capex-spend-concentration-202608"]', {
      timeout: 20000,
    });
    ...
    await clickBtn(page, "Late-Aug 202608");
    ...
    console.log("qa-ai-capex-spend-concentration-202608: PASS");
  } finally {
    await browser.close();
