/**
 * QA: geopolitics-institutions-update-2026
...
const slug = "geopolitics-institutions-update-2026";
...
const htmlPath = fs.existsSync(htmlPathFlat)
  ...
  : fs.existsSync(htmlPathNested)
    ? htmlPathNested
    : htmlPathFlat;
const dataPath = path.join(root, "src", "data", "geopolitics-institutions-update-2026-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "GeopoliticsInstitutionsUpdateDashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "politics-geopolitics-institutions-update-2026-hero.png",
);

const markers = [
  "Vintage delta — research stock",
  "Consent shortfall vs threshold",
  "Quota consent path to May 2026",
  "IBRD misalignment census",
  "Frozen IMF vote−GDP gaps",
  "Reform lever outcomes",
];
...
const wordCount = (() => {
  const start = postsSrc.indexOf('id: "geopolitics-institutions-update-2026"');
  if (start < 0) return 0;
...
const staticChecks = [
  ...
  ["quotaConsentPct 72.78", dataSrc.includes("quotaConsentPct: 72.78")],
  ["quotaGapPp -12.22", dataSrc.includes("quotaGapPp: -12.22")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="geopolitics-institutions-update-2026"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  ["Word count >= 1200", wordCount >= 1200],
  ["H2 count >= 6", h2Count >= 6],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — research stock") || findInChunks("Consent shortfall vs threshold"),
  ],
];

let failed = 0;
for (const [name, ok] of staticChecks) {
  console.log(ok ? "✓" : "✗", name, name.includes("Word") ? `(${wordCount})` : name.includes("H2") ? `(${h2Count})` : "");
  if (!ok) failed++;
}
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="politics-geopolitics-institutions-update-2026-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
