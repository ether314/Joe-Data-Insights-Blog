/**
 * QA: macro-growth-trade-update-2026q3
...
const slug = "macro-growth-trade-update-2026q3";
...
const htmlPath = fs.existsSync(htmlPathFlat)
  ...
  : fs.existsSync(htmlPathNested)
    ? htmlPathNested
    : htmlPathFlat;
const dataPath = path.join(root, "src", "data", "macro-growth-trade-update-2026q3-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "MacroGrowthTradeUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "economics-macro-growth-trade-update-2026q3-hero.png",
);

const markers = [
  "Hard-data check — Jul 2026 IMF baseline",
  "CPB merchandise volume — monthly MoM",
  "Triad tracking — July path vs hard signal",
  "US growth × prices — dual path into Q2",
  "Q2 print vs July IMF annual path",
  "Q1 → Q2 slope",
  "Growth × price scatter",
];
...
const staticChecks = [
  ...
  ["cpbMayMom 1.0", dataSrc.includes("cpbMayMom: 1.0")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="macro-growth-trade-update-2026q3"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Hard-data check — Jul 2026 IMF baseline") || findInChunks("CPB merchandise volume"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="economics-macro-growth-trade-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
