...
const slug = "ai-financing-update-202608";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-financing-update-202608-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiFinancingUpdate202608Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-financing-update-202608-hero.png",
);

const markers = [
  "Vintage delta — Q3 2026 flow print",
  "Flow perimeter vs funded stock vs lease overhang",
  "Aug funded + overhang channels",
  "Hyperscaler bond issuance path",
  "New Aug disclosures (vs Q3)",
];
...
const staticChecks = [
  ...
  ["Headline funded stock 1065", dataSrc.includes("fundedStockTotalBn: 1065")],
  ["Lease overhang 675", dataSrc.includes("uncommencedLeasesBn: 675")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-financing-update-202608"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Q3 2026 flow print") ||
      findInChunks("Flow perimeter vs funded stock vs lease overhang"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-financing-update-202608-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
