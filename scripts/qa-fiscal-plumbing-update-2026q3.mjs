/**
 * QA: fiscal-plumbing-update-2026q3
...
import {
  ...
  TRUST_DELTAS,
  VINTAGE_DELTAS,
} from "../src/data/fiscal-plumbing-update-2026q3-data.ts";

assert.equal(HEADLINE.jctPriorTn, 2.42);
assert.equal(HEADLINE.jctNewTn, 2.51);
assert.equal(HEADLINE.oasdiPriorYear, 2033);
assert.equal(HEADLINE.oasdiNewYear, 2033);
assert.equal(HEADLINE.gsePriorTn, 8.1);
assert.equal(HEADLINE.gseNewTn, 8.35);
assert.equal(HEADLINE.netInterestPriorBn, 1050);
assert.equal(HEADLINE.netInterestNewBn, 1120);
...
const slug = "fiscal-plumbing-update-2026q3";
const markers = [
  "Fiscal plumbing — Q3 vintage delta",
  "Net interest $1.05T → $1.12T",
];
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Interest race" }).first().click();
