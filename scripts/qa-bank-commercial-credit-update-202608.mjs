/**
 * QA: bank-commercial-credit-update-202608
 * // viz-types: signed Δ bars, delinq×SS scatter, maturity bars, DY pie, cohort grouped bars, Fed line, stress scatter | layout: canvas
 */
...
const slug = "bank-commercial-credit-update-202608";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(
  root,
  "src",
  "data",
  "bank-commercial-credit-update-202608-data.ts",
);
const dashboardPath = path.join(
  ...
  "components",
  "visualizations",
  "BankCommercialCreditUpdate202608Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "finance-bank-commercial-credit-update-202608-hero.png",
);

const markers = [
  "Aug vintage · CMBS delinq up / special servicing down · hard maturities",
  "What changed versus the Q3 update",
  "Property-type divergence — delinquency MoM vs special servicing MoM",
  "August hard-maturity mix",
  "Debt-yield risk bands",
  "July → August hard-maturity cohort",
...
const staticChecks = [
  ...
  ["cmbsDelinq 7.86", dataSrc.includes("cmbsDelinq: 7.86")],
  ["augMaturityBn 5.49", dataSrc.includes("augMaturityBn: 5.49")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  [
    "Dashboard wired",
    dashboardSrc.includes('data-viz="bank-commercial-credit-update-202608"'),
  ],
  ...
  [
    ...
    findInChunks(
      "Aug vintage · CMBS delinq up / special servicing down · hard maturities",
    ) || findInChunks("What changed versus the Q3 update"),
  ],
];
...
try {
  ...
  await page.getByRole("button", { name: "Delinq × SS" }).first().click();
  await page
    .getByText("Property-type divergence", { exact: false })
    .first()
    .waitFor({ timeout: 10000 });
  await page.getByRole("button", { name: "Delinq↑ SS↓" }).first().click();
  await page.waitForTimeout(300);
  await page.getByRole("button", { name: "All" }).first().click();
  console.log("✓ Divergence quadrant toggles");

  await page.getByRole("button", { name: "Stress map" }).first().click();
  const bookBank = page.getByRole("button", { name: "Bank SA" });
  if (await bookBank.count()) {
    await bookBank.first().click();
