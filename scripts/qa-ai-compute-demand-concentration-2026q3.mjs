/**
 * QA: ai-compute-demand-concentration-2026q3
...
import {
  ...
  SITE_LEDGER,
  TOKEN_BRANDS,
} from "../src/data/ai-compute-demand-concentration-2026q3-data.ts";

assert.equal(HEADLINE.ownerTop1Pct, 25.0);
assert.equal(HEADLINE.ownerTop3Pct, 54.8);
assert.equal(HEADLINE.cloudTop3Pct, 58);
assert.equal(HEADLINE.usPipelineSiteSharePct, 54.4);
...
const slug = "ai-compute-demand-concentration-2026q3";
const markers = [
  "AI compute demand — Q3 2026 concentration lens",
  "Prior → Q3 concentration deltas",
  "Tokens vs chip ownership",
];
...
async function main() {
  ...
  try {
    ...
    console.log("qa-ai-compute-demand-concentration-2026q3: PASS");
  } finally {
    await browser.close();
