/**
 * QA: geopolitics-institutions-update-2026q3
 * // viz-types: grouped vintage bars, consent path+threshold, stacked gap area, BBA donut, gap×consent scatter, lever outcome bars | layout: default
 */
...
const slug = "geopolitics-institutions-update-2026q3";
...
const dataPath = path.join(
  ...
  "src",
  "data",
  "geopolitics-institutions-update-2026q3-data.ts",
);
const dashboardPath = path.join(
  ...
  "components",
  "visualizations",
  "GeopoliticsInstitutionsUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "politics-geopolitics-institutions-update-2026q3-hero.png",
);

const markers = [
  "Q3 vintage delta — Oct 2025",
  "Prior vs latest consent share",
  "Quota consent path to Nov 2026",
  "Shortfall closed vs remaining",
  "BBA bridge: transitional lending capacity",
  "Frozen IMF vote−GDP gaps",
  "What moved vs stuck since prior post",
];
...
const wordCount = (() => {
  const start = postsSrc.indexOf('id: "geopolitics-institutions-update-2026q3"');
  if (start < 0) return 0;
...
const staticChecks = [
  ...
  ["quotaDeltaPp 3.88", dataSrc.includes("quotaDeltaPp: 3.88")],
  ["quotaGapPp -8.34", dataSrc.includes("quotaGapPp: -8.34")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  [
    "Dashboard wired",
    dashboardSrc.includes('data-viz="geopolitics-institutions-update-2026q3"'),
  ],
  ...
  [
    "Dashboard bundled",
    findInChunks("Q3 vintage delta — Oct 2025") ||
      findInChunks("Prior vs latest consent share"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="politics-geopolitics-institutions-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
