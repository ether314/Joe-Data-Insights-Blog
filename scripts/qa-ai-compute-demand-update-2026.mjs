/**
 * QA: ai-compute-demand-update-2026
...
const slug = "ai-compute-demand-update-2026";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-compute-demand-update-2026-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiComputeDemandUpdateDashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "technology-ai-compute-demand-update-2026-hero.png",
);

const markers = [
  "Vintage delta — Jul research",
  "Ownership vintage change",
  "Prior → new ownership levels (dumbbell)",
  "Hub IT-MW revisions (prior → new)",
  "Geography concentration deltas",
  "Tokens vs ownership (scatter)",
  "Gartner power path — capacity GW & AI server share",
];
...
const staticChecks = [
  ...
  ["Microsoft +2.3 pp", dataSrc.includes("microsoftShareDeltaPp: 2.3")],
  ["US pipeline 45 GW", dataSrc.includes("usPipelineGw: 45")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-compute-demand-update-2026"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Jul research") || findInChunks("Ownership vintage change"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="technology-ai-compute-demand-update-2026-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
