/**
 * QA: ai-financing-concentration-202608
...
import {
  ...
  ISSUER_SHARES,
  ISSUER_SHARE_PATH,
} from "../src/data/ai-financing-concentration-202608-data.ts";

assert.equal(HEADLINE.top1ChannelSharePct, 49);
assert.equal(HEADLINE.top3ChannelSharePct, 91);
...
assert.equal(HEADLINE.uncommencedLeasesBn, 675);
assert.equal(CHANNEL_STOCK_SHARES[0].sharePct, 49);
...
async function main() {
  ...
  try {
    ...
    await clickBtn(page, "Commitment + stress");
    await clickBtn(page, "High (~$140B)");
    ...
    console.log("qa-ai-financing-concentration-202608: PASS");
  } finally {
    await browser.close();
