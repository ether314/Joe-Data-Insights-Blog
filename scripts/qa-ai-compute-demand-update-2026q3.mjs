/**
 * QA: ai-compute-demand-update-2026q3
...
const slug = "ai-compute-demand-update-2026q3";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-compute-demand-update-2026q3-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiComputeDemandUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "technology-ai-compute-demand-update-2026q3-hero.png",
);

const markers = [
  "Vintage delta — Aug 2026 update",
  "Hyperscale sites — US vs rest of world",
  "Gartner power split — AI crosses conventional in 2027",
  "Hub IT-MW revisions (Aug → Q3)",
  "Geography concentration deltas",
  "Ownership carry (Aug explorer → Q3)",
...
const staticChecks = [
  ...
  ["US pipeline sites 437", dataSrc.includes("usPipelineSites: 437")],
  ["Top-3 share 58", dataSrc.includes("top3HyperscaleSharePct: 58")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-compute-demand-update-2026q3"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Aug 2026 update") ||
      findInChunks("Hyperscale sites — US vs rest of world"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="technology-ai-compute-demand-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
