...
const slug = "ai-financing-update-2026q3";
const htmlPath = path.join(root, "out", "blog", `${slug}.html`);
const dataPath = path.join(root, "src", "data", "ai-financing-update-2026q3-data.ts");
const dashboardPath = path.join(
  root,
  "src",
  "components",
  "visualizations",
  "AiFinancingUpdate2026q3Dashboard.tsx",
);
const heroPath = path.join(
  root,
  "public",
  "images",
  "capital-markets-ai-financing-update-2026q3-hero.png",
);

const markers = [
  "Vintage delta — Aug mid-year",
  "AI theme share of US credit supply",
  "Vintage deltas that moved",
  "New Q3 disclosures",
  "Equity channel (2025 ETF flows)",
];
...
const staticChecks = [
  ...
  ["Headline IG 23%", dataSrc.includes("aiIgShareNewPct: 23")],
  ["Dashboard exists", fs.existsSync(dashboardPath)],
  ["Dashboard wired", dashboardSrc.includes('data-viz="ai-financing-update-2026q3"')],
  ["Hero PNG exists", fs.existsSync(heroPath)],
  [
    "Dashboard bundled",
    findInChunks("Vintage delta — Aug mid-year") || findInChunks("AI theme share of US credit supply"),
  ],
];
...
try {
  ...
  const heroVisible = await page
    .locator('img[src*="capital-markets-ai-financing-update-2026q3-hero"]')
    .first()
    .isVisible()
    .catch(() => false);
  if (heroVisible) {
